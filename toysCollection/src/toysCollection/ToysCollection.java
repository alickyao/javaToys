 ...
 * 
 * 容器
 * ArrayList 类似数组 读的快  改起来慢
 * LinkedList 读的慢 但是改起来快
 * Set        上面两者之间
 * 容器就用 List Set 和  Map 这些都是父类指向子类的对象
 */
public class ToysCollection {
	@SuppressWarnings({ "rawtypes", "unchecked", "unused" })//通知编译器忽略这些警告，因为已经确认是没有问题的
	public static void main(String[] args){
		...
		//增强的for循环
		for(Object cObject : list){
			System.out.println(cObject);//这里会JAVA会执行对象的重写的toString()方法，多态的提现
		}
		...
		list1.add(new Persion("范晓旭", 20));
		list1.add(new Persion("范wang旭", 35));
		...
		int s = Collections.binarySearch(list1,new Persion("a", 40));//折半查找，先要排序，正序然后才能用，不怎么好用（*如果里面排序依据都一样，那折半就用不了应为他也不知道中间在哪里）
		...
		//HashMap 键值对形式出现的
		Map m1 = new HashMap();
		m1.put("wangyoucai", new Persion("wangyoucai", 62));
		m1.put("lizhangfei", new Persion("lizhangfei", 61));
		System.out.println(m1);
		
		Object w = m1.get("wangyoucai");//get方法返回的是他的value
		System.out.println(w);
		
		System.out.println(m1.containsKey("wangyoucai")); //是否有某个 key
		
		System.out.println(m1.containsValue(new Persion("lizhangfei", 61)));//也可以比较是否有某个value 这里还是调用的类的equals() 来实现的
		

		Map m2 = new HashMap();//可以将m1直接复制给m2
		m2.put("s", 1);//自动打包
		
		int i = (Integer)m2.get("s");//自动解包
	}
}

@SuppressWarnings("rawtypes")
class Persion implements Comparable{//让类实现能够比较的接口 用于排序Collections.sort
	String name;
	int age;
	 ...
	 * 因为遍历比较使用equals没有效率，所以要用hashCode()，返回的Int来比较 
